...
share_w1 <- read_dta(paste0(base_path, "SHARE/output/cluster_assignments/gender/share_w1m.dta"))
share_w6 <- read_dta(paste0(base_path, "SHARE/output/cluster_assignments/gender/share_w6m.dta"))
hrs_w7 <- read_dta(paste0(base_path, "HRS/output/cluster_assignments/gender/hrs_w7m.dta"))
hrs_w12 <- read_dta(paste0(base_path, "HRS/output/cluster_assignments/gender/hrs_w12m.dta"))
elsa_w2 <- read_dta(paste0(base_path, "ELSA/UKDA-5050-stata/output/cluster_assignments/gender/elsa_w2m.dta"))
elsa_w8 <- read_dta(paste0(base_path, "ELSA/UKDA-5050-stata/output/cluster_assignments/gender/elsa_w8m.dta"))
mhas_w2 <- read_dta(paste0(base_path, "MHAS/output/cluster_assignments/gender/mhas_w2m.dta"))
mhas_w4 <- read_dta(paste0(base_path, "MHAS/output/cluster_assignments/gender/mhas_w4m.dta"))
klosa_w3 <- read_dta(paste0(base_path, "KLoSA/output/cluster_assignments/gender/klosa_w3m.dta"))
klosa_w8 <- read_dta(paste0(base_path, "KLoSA/output/cluster_assignments/gender/klosa_w8m.dta"))
charls_w1 <- read_dta(paste0(base_path, "CHARLS/output/cluster_assignments/gender/charls_w1m.dta"))
charls_w4 <- read_dta(paste0(base_path, "CHARLS/output/cluster_assignments/gender/charls_w4m.dta"))
# Set working directory
setwd(paste0(base_path, "Clustering/Analysis/male"))
jco_colors <- c("#2c7bb6", "#519fcb", "#81b5d3", "#c1cfb9", "#f6d1a5", "#f09068", "#f46d43", "#d73027")
#-------------------Health status-------------------
# Set output directory
output_path <- file.path(base_path, "Clustering/Analysis/male/Health status/")
...
exclude_vars_charlsw1 <- c("h1itot_constant", "r1agey") # Variables to exclude
...
output_folder <- "C:/Users/Pietro/OneDrive - Universita' degli Studi di Roma Tor Vergata/RA/Clustering/Analysis/MEANS"
...
cluster1_data <- hrs_w12 %>%
  filter(cluster == 2) %>%
  select(r12agey_e)

cluster2_data <- hrs_w12 %>%
  filter(cluster == 3) %>%
  select(r12agey_e)

# cluster3_data <- share_w1 %>%
#   filter(cluster == 6) %>%
#   select(h1itot_constant)
# 
# cluster4_data <- share_w1 %>%
#   filter(cluster == 8) %>%
#   select(h1itot_constant)

# Combine data with a new column indicating the cluster
combined_data <- bind_rows(
  mutate(cluster1_data, cluster = "Cluster 2"),
  mutate(cluster2_data, cluster = "Cluster 6"),
  # mutate(cluster3_data, cluster = "Cluster 6"),
  # mutate(cluster4_data, cluster = "Cluster 8")
)

# Plot density distributions
p <-ggplot(combined_data, aes(x = r12agey_e, fill = cluster)) +
  geom_density(alpha = 0.5) +
  labs(title = "Income Density Distribution by Cluster",
       x = "Income",
       y = "Density") +
  scale_x_continuous(labels = scales::comma) + # Format x-axis
  scale_fill_manual(values = c("Cluster 2" = "#1a4771", "Cluster 3" = "#aaba95")) +
  theme_minimal() + 
  theme(
...
# output_folder <- "/Users/pietromonti/Library/CloudStorage/OneDrive-Universita'degliStudidiRomaTorVergata/RA/Clustering/Analysis/SHARE"
# 
# # Ensure the directory exists
# if (!dir.exists(output_folder)) {
#   dir.create(output_folder, recursive = TRUE)
...
# output_file <- file.path(output_folder, "Income_Density_Distribution.pdf")
...
ggsave("/Users/pietromonti/Library/CloudStorage/OneDrive-Universita'degliStudidiRomaTorVergata/RA/Clustering/Analysis/male/Increase_in_Joint_Prevalence.pdf",
       width = 10, height = 8)
#-------
...
ggplot(long_data, aes(x = Pair, y = value, fill = variable)) +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave("/Users/pietromonti/Library/CloudStorage/OneDrive-Universita'degliStudidiRomaTorVergata/RA/Clustering/Analysis/male/Contribution_of_Morbidities.pdf",
       width = 10, height = 8)
